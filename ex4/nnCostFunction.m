...
for i=1:num_labels,
    J += -(1/m)*...
        ...
        (y==i)'*log(
                    sigmoid(
                            [ones(m,1),sigmoid([ones(m,1),X]*Theta1')]*Theta2'
                            )
                    )(:,i)+...
        (1-(y==i))'*(
                      log(1-sigmoid([ones(m,1),sigmoid([ones(m,1),X]*Theta1')]*Theta2')(:,i))
                      )
        );
end;
...
for i = 1:m,
    ...
    error_3 = a3 - outlayer(y(i),num_labels);   
...
L1=eye(size(Theta1_grad,2));
L1(1,1)=0;
L2=eye(size(Theta2_grad,2));
L2(1,1)=0;

Theta1_grad = 1/m.*Theta1_grad+(lambda/m).*Theta1*L1;
Theta2_grad = 1/m.*Theta2_grad+(lambda/m).*Theta2*L2;
